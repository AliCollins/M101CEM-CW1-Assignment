...
Within this same paper, de Leeuw indicates that there is ``potential for the pressure loss measurement...to use it as a means to determine the liquid content of the flow, from which the overreacting factor can be determined accordingly. In essence this would form a simple two-phase flow meter.''  This \acrshort{PLR} is a ratio of the overall difference in the pressure across the Venturi meter, $DP_{t}$ to that of the normal Venturi differential pressure, $DP_{v}$  such that:
\begin{equation}
...
\todo[inline]{Why is downstream tapping at 6D?  The pressure loss tapping appears to be at 6D to match that of orifice meters; but they have the vena contracta downstream of the device, such that the pressure recovery starts after that.  With the Venturi, the walls determine the shape of the flow, such that the flow is returned to similar state directly after the Venturi. 6D may therefore be a technological hangover from a different device.} 
\todo[inline]{Reference for SISA use of 0D, such as \cite{Collins2017PLR1}}
\todo[inline]{Included angle of cones - 21 degrees for convergent (5.2.3 of \cite{ISOTubes}), 7 to 15 degrees for divergent (5.2.5 of \cite{ISOTubes})}
...
$H = 1.0$ for hydrocarbon liquid, $H = 1.35$ for water, $H = 0.79$ for liquid water in wet-steam flow.
