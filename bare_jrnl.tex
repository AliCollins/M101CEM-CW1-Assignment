...
\todo[inline]{Should the discussion of 0D vs 6D be moved lower in the paper?}
...
\todo[inline]{Doe the performance of a similar system demonstrate whether they are the right sort of solution, with the right key characteristics?}

\section{Solution}

Solartron ISA have an extensive wet gas calibration database for Venturi meters, which has previously been used to assess the performance of publicly available standard wet gas correlation algorithms \cite{Collins2015}.  Of relevance to this paper are \hl{XXX} wet gas calibrations (consisting of \hl{XXX} test points in total, for \hl{XXX} flow meters between 3" and 10") for Dualstream 1 products which have a downstream tapping at $0D$ as previously described.


\todo[inline]{How will the comparison take place?}

As per previous work \cite{Collins2015}, where the $H$ parameter is required in the ISO/TR~11583:2012  \cite{2012ISO/TRConduits} equations, the following formulation is used:

\begin{equation}
H = 1.0 + 0.35 \frac{Q_{mw}}{Q_{ml}}    
\end{equation}
This makes the correct assumption that none of the data in the Solartron ISA database is for steam flow, and assumes a nominal straight line fit between the liquid being all hydrocarbon (where the water mass flow rate $Q_{mw}$ is zero) and all water.

\section{Experimentation}

\todo[inline]{Naive fitting of SISA data using ISO/TR 11583 algorithm for PLR}

\begin{figure}[h]
\centering
\includegraphics[width=3.5in]{PLR1.png}
\caption[]{ Measured PLR vs ISO/TR 11583, coloured by meter size }
\label{fig:PLR1}
\end{figure}

\todo[inline]{Adjusted parameters for better fit - does the general shape of the equations work?}
\todo[inline]{Dry gas calibration points}
